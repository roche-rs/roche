...
#[test]
fn generate_build_function_file_only() {
    let path = root("generate_build_function_file_only");
    ...
    let mut build_cmd = Command::new(&me)
        .arg("build")
        .arg("-t")
        .arg("generates_function_file_only")
        .current_dir(&path)
        .spawn()
        .unwrap();
...
#[test]
fn generate_build_project() {
    let path = root("generate_build_project");
    if Path::new(&path).exists() {
        remove_dir_all(&path).unwrap();
    }
    fs::create_dir_all(&path).expect(&format!("couldn't create {:?} directory", path.display()));
